...
module.exports = {
  baseOnPictureCount(volSize) {
    ...
    let picIndex = 1;
    let volIndex = 1;
    const { comicName, saveDir } = store.get();
    const comicDir = helper.mkDir(saveDir, comicName);
    const volComicDir = helper.mkDir(saveDir, `${comicName}(vol_pre_${volSize}pics)`);
    const chapters = helper.getDirContent(comicDir);
  ...
  baseOnChapterCount(volSize) {
    ...
    const { comicName, saveDir } = store.get();
    const comicDir = helper.mkDir(saveDir, comicName);
    const volComicDir = helper.mkDir(saveDir, `${comicName}(vol_pre_${volSize}chapters)`);
    const chapters = helper.getDirContent(comicDir);
