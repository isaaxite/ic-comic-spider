...
exports.crop = async () => {
  let baseNum = 0;
  const { cropDir: chapterDirs, outDir, comicName } = store.get();
  const saveDir = path.join(outDir, `${comicName}_crop`);
  ...
  for (const chapterDir of chapterDirs) {
    ...
    const spin = ora(`crop ${chapterName}...`).start();
    const picNameInfos = helper.getDirContent(chapterDir);
    for (const picNameInfo of picNameInfos) {
      const format = picNameInfo.name.split('.').pop();
      const filePath = path.join(chapterDir, picNameInfo.name);
      const saveChapterDir = path.join(saveDir, chapterName);
      const result = await crop.init(filePath);
      if (!fs.existsSync(saveChapterDir)) {
        fs.mkdirSync(saveChapterDir);
      }
      if (result.length > 1) {
        const streams = result;
        streams.forEach((stream, index) => {
          const filename = picNameInfo.index + index + baseNum;
          const savePath = path.join(saveChapterDir, `./${filename}.${format}`);
          const writeStream = fs.createWriteStream(savePath);
          stream.pipe(writeStream);
        });
        baseNum += (streams.length - 1);
      } else {
        const data = result.pop();
        const filename = +picNameInfo.index + baseNum;
        const savePath = path.join(saveChapterDir, `./${filename}.${format}`);
        fs.writeFileSync(savePath, data);
      }
    }
    baseNum = 0;
    spin.succeed(`${chapterName} finish!`);
    // console.log(picNameInfos);
  }
};
